...
\begin{center}
    \begin{tikzpicture}{node distance = 1cm, auto}
        ...
        \draw[line] (C) --node[above]{} (P);
        \draw[line] (C) --node[above]{} (M);
        \draw[line] (M) --node[above]{} (P);
        \draw[line] (P) --node[above]{hP} (rightofP);
        \draw[line] (P) --node[left]{} (aboveofP);
        \draw[line] (C) --node[left]{dC} (aboveofC)
        \begin{scope}[transform canvas = {xshift = -.15cm}]
            \draw[line](C) -- node[left]{$rTC$} (T);
        \end{scope}
        \begin{scope}[transform canvas = {xshift = .15cm}]
            \draw[line](T) --node[right]{\mu_{1}C} (C);        
        \end{scope}
        \begin{scope}[transform canvas = {yshift = .15cm}]
            \draw[line] (T) --node[above]{$\gammaMT$} (M);
        \end{scope}
        \begin{scope}[transform canvas = {xshift = -.15cm}]
            \draw[line] (M) --node[below]{gMP} (T);
        \end{scope}
    \end{tikzpicture}
\end{center}
