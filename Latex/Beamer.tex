...
\begin{table}
    ...
    \begin{tabular}{c p{5.5cm} c c}
        ...
        $\sigma$ & rate that parrot fish bite coral\textsuperscript{\cite{mcdermott_2021}}  & 0.01*& $bites \cdot year^{-1}$\\
...
\begin{frame}{Simulating $a(t)$}
    ...
    \begin{figure}
        \centering
        \animategraphics[width=0.75\textwidth, loop, autoplay, palindrome]{5}{Latex/Figures/matlab_a_animation/Frame-}{1}{50}
...
\begin{frame}{Disease Free Equilibrium}
    \begin{align*}
        ...
        M^{0} &= 0\\
        P^{0} &= -\frac{\beta(1 - \frac{\mu_{1}}{r})(h - \mu_{2} - q)}{q}\\
        %\Longleftarrow \text{our "disease" compartment}
    \end{align*}
    %\begin{block}{Note}
...
\begin{frame}{Basic Reproduction Number: $\mathscr{R}_{0}$}
    \begin{block}{Definition}
        \begin{itemize}
            ...
            \item $\mathscr{R}_{0}$ is defined as the expected number of secondary cases produced a single macroalgae (M) in a completely coral (C) population. % def in terms of our model
        \end{itemize}
    \end{block}
\end{frame}
...
\begin{frame}{Harvesting Threshold}
    \begin{block}{Definition (Harvesting Threshold)}
        The proportion of parrotfish that can be harvested in order for macroalgae growth maintain stability in the ecosystem. 
    \end{block}
...
\begin{frame}{Future Research}
    \begin{itemize}
    \item Collect data from Guam sources to give a more accurate representation of the changes on Guam's Reef Ecosystem. 
