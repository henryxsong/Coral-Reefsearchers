...
% For-loop for animating parameter change
for i = 1:length(t)
    a0 = i/length(t);
    a = @(t) abs((a0*(9*sin(pi*t)+1))/(10));
    ...
    % Plot
    txt = ['a_0 = ' num2str(a0)]; % shows value of param value at current iteration
    ...
    % stored)
    filename = append('Frame-', num2str(i)); %file name of current iteration
    saveas(fig, filename, 'png'); %save figure as .png
end
